...
iptables -t filter -A OUTPUT -o eth0+ -d 10.0.0.0/8 -j ACCEPT -m comment -m comment --comment 'to local /8 net'
iptables -t filter -A OUTPUT -o eth0+ -d 172.16.0.0/12 -j ACCEPT -m comment -m comment --comment 'to local /12 net'
iptables -t filter -A OUTPUT -o eth0+ -d 192.168.0.0/16 -j ACCEPT -m comment -m comment --comment 'to local /16 net'
...
exec openvpn --config /etc/openvpn.conf --remote $VPN_IP
